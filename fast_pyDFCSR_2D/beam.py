...
class Beam():
    ...
    def apply_wakes(self, dE_vals, x_kick_vals, csr_params, t2_csr, R_inv_csr, step_size):
        """
        ...
        interp = RegularGridInterpolator((o_dim, p_dim), x_kick_vals, fill_value=0.0, bounds_error=False)
        x_kick_per_particle = interp(particle_positions)
    ...
    def get_std_wrt_linear_fit(self):
        """
        ...
        parallel_std = np.std(np.sqrt(proj_x_line**2 + proj_z_line**2) * np.sign(proj_x_line))
    ...
    def scatterplot_beam(self):
        ...
        #ransac.fit(self.z.reshape(-1, 1), self.x)

        # Get the slope and intercept from the RANSAC model
    ...
    @property
    def tilt_angle(self):
        return math.atan(self.linear_fit.slope)
    ...
    def x_transform(self):
        ...
        (will make a tilted distribution horizontal in x direction that lies on the z axis)
        """
        linear_fit = self.linear_fit
        polynomial = np.poly1d(linear_fit.slope, linear_fit.intercept)
        return self.x - np.polyval(polynomial, self.z)
